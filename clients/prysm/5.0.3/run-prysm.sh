...
append_option "--accept-terms-of-use" "$PRSYM_CLI_ACCEPT_TERMS_OF_USE"
...
append_option "--disable-monitoring" "$PRSYM_CLI_DISABLE_MONITORING"
append_option "--e2e-config" "$PRSYM_CLI_E2E_CONFIG"
append_option "--enable-tracing" "$PRSYM_CLI_ENABLE_TRACING"
...
append_option "--enable-debug-rpc-endpoints" "$PRSYM_CLI_ENABLE_DEBUG_RPC_ENDPOINTS"
append_option "--enable-experimental-backfill" "$PRSYM_CLI_ENABLE_EXPERIMENTAL_BACKFILL"
...
append_option "--historical-slasher-node" "$PRSYM_CLI_HISTORICAL_SLASHER_NODE"
...
append_option "--subscribe-all-subnets" "$PRSYM_CLI_SUBSCRIBE_ALL_SUBNETS"
...
append_option "--enable-upnp" "$PRSYM_CLI_ENABLE_UPNP"
...
append_option "--p2p-static-id" "$PRSYM_CLI_P2P_STATIC_ID"
...
append_option "--blob-save-fsync" "$PRSYM_CLI_BLOB_SAVE_FSYNC"
append_option "--dev" "$PRSYM_CLI_DEV"
append_option "--disable-broadcast-slashings" "$PRSYM_CLI_DISABLE_BROADCAST_SLASHINGS"
append_option "--disable-eip-4881" "$PRSYM_CLI_DISABLE_EIP_4881"
append_option "--disable-grpc-connection-logging" "$PRSYM_CLI_DISABLE_GRPC_CONNECTION_LOGGING"
append_option "--disable-peer-scorer" "$PRSYM_CLI_DISABLE_PEER_SCORER"
append_option "--disable-registration-cache" "$PRSYM_CLI_DISABLE_REGISTRATION_CACHE"
append_option "--disable-resource-manager" "$PRSYM_CLI_DISABLE_RESOURCE_MANAGER"
append_option "--disable-staking-contract-check" "$PRSYM_CLI_DISABLE_STAKING_CONTRACT_CHECK"
append_option "--disable-verbose-sig-verification" "$PRSYM_CLI_DISABLE_VERBOSE_SIG_VERIFICATION"
append_option "--enable-experimental-state" "$PRSYM_CLI_ENABLE_EXPERIMENTAL_STATE"
append_option "--enable-full-ssz-data-logging" "$PRSYM_CLI_ENABLE_FULL_SSZ_DATA_LOGGING"
append_option "--enable-historical-state-representation" "$PRSYM_CLI_ENABLE_HISTORICAL_STATE_REPRESENTATION"
append_option "--enable-lightclient" "$PRSYM_CLI_ENABLE_LIGHTCLIENT"
append_option "--holesky" "$PRSYM_CLI_HOLESKY"
append_option "--interop-write-ssz-state-transitions" "$PRSYM_CLI_INTEROP_WRITE_SSZ_STATE_TRANSITIONS"
append_option "--mainnet" "$PRSYM_CLI_MAINNET"
append_option "--prater" "$PRSYM_CLI_PRATER"
append_option "--prepare-all-payloads" "$PRSYM_CLI_PREPARE_ALL_PAYLOADS"
append_option "--save-full-execution-payloads" "$PRSYM_CLI_SAVE_FULL_EXECUTION_PAYLOADS"
append_option "--save-invalid-blob-temp" "$PRSYM_CLI_SAVE_INVALID_BLOB_TEMP"
append_option "--save-invalid-block-temp" "$PRSYM_CLI_SAVE_INVALID_BLOCK_TEMP"
append_option "--sepolia" "$PRSYM_CLI_SEPOLIA"
append_option "--slasher" "$PRSYM_CLI_SLASHER"
append_option "--genesis-state" "$PRSYM_CLI_GENESIS_STATE"
