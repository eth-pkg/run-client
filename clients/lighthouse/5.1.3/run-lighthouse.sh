...
append_option "--allow-insecure-genesis-sync" "$LIGHTHOUSE_CLI_BN_ALLOW_INSECURE_GENESIS_SYNC"
append_option "--always-prefer-builder-payload" "$LIGHTHOUSE_CLI_BN_ALWAYS_PREFER_BUILDER_PAYLOAD"
append_option "--always-prepare-payload" "$LIGHTHOUSE_CLI_BN_ALWAYS_PREPARE_PAYLOAD"
append_option "--builder-fallback-disable-checks" "$LIGHTHOUSE_CLI_BN_BUILDER_FALLBACK_DISABLE_CHECKS"
append_option "--compact-db" "$LIGHTHOUSE_CLI_BN_COMPACT_DB"
append_option "--disable-backfill-rate-limiting" "$LIGHTHOUSE_CLI_BN_DISABLE_BACKFILL_RATE_LIMITING"
append_option "--disable-deposit-contract-sync" "$LIGHTHOUSE_CLI_BN_DISABLE_DEPOSIT_CONTRACT_SYNC"
append_option "--disable-duplicate-warn-logs" "$LIGHTHOUSE_CLI_BN_DISABLE_DUPLICATE_WARN_LOGS"
append_option "--disable-enr-auto-update" "$LIGHTHOUSE_CLI_BN_DISABLE_ENR_AUTO_UPDATE"
append_option "--disable-lock-timeouts" "$LIGHTHOUSE_CLI_BN_DISABLE_LOCK_TIMEOUTS"
append_option "--disable-log-timestamp" "$LIGHTHOUSE_CLI_BN_DISABLE_LOG_TIMESTAMP"
append_option "--disable-malloc-tuning" "$LIGHTHOUSE_CLI_BN_DISABLE_MALLOC_TUNING"
append_option "--disable-optimistic-finalized-sync" "$LIGHTHOUSE_CLI_BN_DISABLE_OPTIMISTIC_FINALIZED_SYNC"
append_option "--disable-packet-filter" "$LIGHTHOUSE_CLI_BN_DISABLE_PACKET_FILTER"
append_option "--disable-proposer-reorgs" "$LIGHTHOUSE_CLI_BN_DISABLE_PROPOSER_REORGS"
append_option "--disable-quic" "$LIGHTHOUSE_CLI_BN_DISABLE_QUIC"
append_option "--disable-upnp" "$LIGHTHOUSE_CLI_BN_DISABLE_UPNP"
append_option "--dummy-eth1" "$LIGHTHOUSE_CLI_BN_DUMMY_ETH1"
append_option "--enable-private-discovery" "$LIGHTHOUSE_CLI_BN_ENABLE_PRIVATE_DISCOVERY"
append_option "--enr-match" "$LIGHTHOUSE_CLI_BN_ENR_MATCH"
append_option "--eth1" "$LIGHTHOUSE_CLI_BN_ETH1"
append_option "--eth1-purge-cache" "$LIGHTHOUSE_CLI_BN_ETH1_PURGE_CACHE"
append_option "--genesis-backfill" "$LIGHTHOUSE_CLI_BN_GENESIS_BACKFILL"
append_option "--gui" "$LIGHTHOUSE_CLI_BN_GUI"
append_option "--http" "$LIGHTHOUSE_CLI_BN_HTTP"
append_option "--http-allow-sync-stalled" "$LIGHTHOUSE_CLI_BN_HTTP_ALLOW_SYNC_STALLED"
append_option "--http-enable-tls" "$LIGHTHOUSE_CLI_BN_HTTP_ENABLE_TLS"
append_option "--import-all-attestations" "$LIGHTHOUSE_CLI_BN_IMPORT_ALL_ATTESTATIONS"
append_option "--light-client-server" "$LIGHTHOUSE_CLI_BN_LIGHT_CLIENT_SERVER"
append_option "--log-color" "$LIGHTHOUSE_CLI_BN_LOG_COLOR"
append_option "--logfile-compress" "$LIGHTHOUSE_CLI_BN_LOGFILE_COMPRESS"
append_option "--logfile-no-restricted-perms" "$LIGHTHOUSE_CLI_BN_LOGFILE_NO_RESTRICTED_PERMS"
append_option "--metrics" "$LIGHTHOUSE_CLI_BN_METRICS"
append_option "--private" "$LIGHTHOUSE_CLI_BN_PRIVATE"
append_option "--proposer-only" "$LIGHTHOUSE_CLI_BN_PROPOSER_ONLY"
append_option "--purge-db" "$LIGHTHOUSE_CLI_BN_PURGE_DB"
append_option "--reconstruct-historic-states" "$LIGHTHOUSE_CLI_BN_RECONSTRUCT_HISTORIC_STATES"
append_option "--reset-payload-statuses" "$LIGHTHOUSE_CLI_BN_RESET_PAYLOAD_STATUSES"
append_option "--shutdown-after-sync" "$LIGHTHOUSE_CLI_BN_SHUTDOWN_AFTER_SYNC"
append_option "--slasher" "$LIGHTHOUSE_CLI_BN_SLASHER"
append_option "--staking" "$LIGHTHOUSE_CLI_BN_STAKING"
append_option "--subscribe-all-subnets" "$LIGHTHOUSE_CLI_BN_SUBSCRIBE_ALL_SUBNETS"
append_option "--validator-monitor-auto" "$LIGHTHOUSE_CLI_BN_VALIDATOR_MONITOR_AUTO"
append_option "--version" "$LIGHTHOUSE_CLI_BN_VERSION"
append_option "--zero-ports" "$LIGHTHOUSE_CLI_BN_ZERO_PORTS"
